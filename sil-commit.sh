...
# SHOW COMMIT MESSAGE
# question "Add all files?" "y/N"
read -p " " COMMIT_ANSWER
N='n';
ADD_ALL="${COMMIT_ANSWER:-$N}"
...
COMMIT_MSG="[$BRANCH_TYPE]: $MESSAGE"


if [ "$ADD_ALL" = 'n' ] || [ "$ADD_ALL" = 'N' ]; then 
    COMMIT_FULL_MSG="git commit -m \"${COMMIT_MSG}\""
else
    COMMIT_FULL_MSG="git commit -am \"${COMMIT_MSG}\"
fi
